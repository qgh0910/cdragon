...
class CloudReranker:
    ...
    使用 DashScope 等云端 API 提供重排序服务，无需下载本地模型
    优点：
    - 无需下载大型模型文件
    - 启动速度快
    - 始终使用最新版本
    ...
    def __init__(
        ...
        self.base_url = (
            base_url
            or settings.cloud_reranker_base_url
    ...
    def _rerank_endpoint(self) -> str:
        """根据模型选择云端 rerank endpoint。"""
        endpoint = "reranks" if self.model == "qwen3-rerank" else "rerank"
