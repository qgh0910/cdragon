...
- 默认由 Codex 负责需求澄清、需求说明、技术方案、验收标准和最终验收；除非用户在最新消息中明确授权，Codex 不进入业务代码实施。
- 默认由 WorkBuddy 负责代码开发、测试编写、本地调试、问题修复和翻译数据生成，并按已确认的需求与技术方案执行。
- 当前国际化翻译初稿优先使用 WorkBuddy 中的 HY3；模型和免费额度不是长期架构依赖，未来更换模型须由用户重新确认。
- WorkBuddy 实施时应更新对应功能的 `PROJECT_PROGRESS.md`，记录修改文件、测试命令、结果、错误和遗留风险，方便 Codex 后续基于实际 diff 和证据验收。
- Codex 验收默认只读检查实际代码、文档、Git diff、测试结果和运行行为；如验收发现问题，应先报告，不在同一验收阶段直接修改业务代码。
- 用户最新消息可以覆盖上述默认分工和阶段边界。
