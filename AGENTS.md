...
- 如果存在 `my_docs/PROJECT_PROGRESS.md`，开始前先阅读；如果任务是阶段性开发且该文件不存在，可创建并维护。
- 每次结束阶段性开发任务前，更新 `my_docs/PROJECT_PROGRESS.md`，记录完成内容、验证方式、遗留问题和下一步建议。
...
- 输出内容应结构化，方便保存到 `my_docs/` 或对应阶段文档。
...
新产出的需求说明、技术方案、设计评审、最终方案、实施计划或阶段性 plan 应保存到 `my_docs/` 目录，文件名建议包含日期和主题，例如 `my_docs/2026-06-08-法律多智能体RAG优化-plan.md`。通用阶段模板统一保存在 `my_docs/prompts/`。
...
由于项目可能分多天推进，为避免上下文丢失，阶段性开发任务应维护：

```text
my_docs/PROJECT_PROGRESS.md
```

所有新产出的设计 plan、实施 plan、阶段性开发计划也应保存到 `my_docs/` 目录，不要默认散落在项目根目录或其他文档目录。
...
如果本次只是简单问答、文档审阅或一次性小修改，可以不更新进度文件，但最终回复中应说明本次未更新的原因。
